...
async function captureDiff(cwd: string): Promise<string> {
  ...
  try {
    await execFileP('git', ['fetch', '-q', 'origin', 'main', '--depth=1'], { cwd });
    const r = await execFileP('git', ['diff', 'origin/main...HEAD', '--no-color'], {
      cwd,
    ...
    try {
      const r = await execFileP('git', ['diff', 'HEAD~1...HEAD', '--no-color'], {
        cwd,
        maxBuffer: 64 * 1024 * 1024,
      });
      return r.stdout;
...
async function runDockerEval(
  ...
  const applied = stdout.includes('PATCH_APPLIED');
