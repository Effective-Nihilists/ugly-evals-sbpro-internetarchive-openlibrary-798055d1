...
def map_data(entry) -> dict[str, Any]:
    ...
    marc_lang_code = 'eng' if entry['dcterms_language'].startswith('en-') else None
    if not marc_lang_code:
        raise ValueError(f"Feed entry language {entry['dcterms_language']} is not supported.")
    import_record = {
        "title": entry['title'],
        "source_records": [f"standard_ebooks:{std_ebooks_id}"],
        "publishers": [entry['dcterms_publisher']],
    ...
    if image_uris:
        import_record['cover'] = f"{BASE_SE_URL}{next(iter(image_uris))['href'].removeprefix(BASE_SE_URL)}"
