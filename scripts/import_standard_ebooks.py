...
def map_data(entry) -> dict[str, Any]:
    ...
    language = entry.get('dcterms_language', entry.get('language', ''))
    ...
    import_record = {
        ...
        "publishers": [entry.get('dcterms_publisher', entry.get('publisher', ''))],
        "publish_date": entry.get('published', entry.get('dc_issued', ''))[0:4],
    ...
    if image_uris:
        import_record['cover'] = next(iter(image_uris))["href"]
